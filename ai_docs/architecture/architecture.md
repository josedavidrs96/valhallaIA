...
- Example: `SqlUserRepository implements UserRepositoryInterface`
...
- Transform DTOs to API Resources
...
user = queryBus.query(new GetUserByIdQuery(userId))

// Trigger action in another BC
commandBus.dispatch(new CreateInvoiceCommand(invoiceData))
```
...
Example: Get invoices with user data
```
// 1. Get invoices from Billing BC
invoices = invoiceQuery.findByOrganization(organizationId)

// 2. Extract user IDs
userIds = invoices.map(invoice => invoice.userId)

// 3. Get all users in ONE query from User BC
users = userQuery.findByIds(userIds)

// 4. Join in application code (not in database)
for invoice in invoices:
    invoice.user = users[invoice.userId] ?? null
```
...
- **Simple ValueObjects** - Example: `UserId`, `OrganizationId`
- **Enums** - Example: `OrderStatus`, `PaymentStatus`
...
```
user = queryBus.query(new GetUserQuery(id))
```
...
```
// From any BC, get user from User BC
user = queryBus.query(new GetUserByIdQuery(userId))
```
...
- **Database** - Large database (always consider performance)
...
1. **Verify access** - Check permissions
2. **Dispatch Command/Query** - Delegate to Application layer
3. **Return response** - Convert to Resource via ResService
...
```
// WRONG
function invoke(AddItemsDto dto):
...
class AddOrderItemsAction:
    ...
    function invoke(dto: AddOrderItemsDto, authPayload: AuthPayload): OrderRes
        // 1. Verify access (ONLY security check)
        order = queryBus.query(new GetOrderByIdQuery(dto.id))
        verifyAccess(authPayload, order.organizationId)
        ...
        return resService.getOrderResource(dto.id)
```
...
| Update totals | Handler + Entity | Business logic |
