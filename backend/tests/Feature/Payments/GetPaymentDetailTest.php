...
final class GetPaymentDetailTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_admin_can_get_payment_detail(): void
    {
        ...
        $paymentId  = $this->insertPayment($memberData['memberId'], $planId, $admin->id, '2026-06');

        $response = $this->actingAs($admin, 'sanctum')
            ->getJson('/api/admin/payments/' . $paymentId);
    ...
    public function test_unknown_id_returns_404(): void
    {
        ...
        $response = $this->actingAs($admin, 'sanctum')
            ->getJson('/api/admin/payments/' . (string) new Ulid());
