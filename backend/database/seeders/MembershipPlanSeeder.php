    ...
    public function run(): void
    {
        $plans = [
            [
                ...
                MembershipPlanTable::MAX_WEEKLY_SESSIONS => 2,
            ],
            [
                ...
                MembershipPlanTable::MAX_WEEKLY_SESSIONS => 3,
            ],
            [
        ...
        foreach ($plans as $plan) {
            $exists = DB::table(MembershipPlanTable::TABLE_NAME)
                ->where(MembershipPlanTable::SLUG, $plan[MembershipPlanTable::SLUG])
                ->exists();

            if ($exists) {
                DB::table(MembershipPlanTable::TABLE_NAME)
                    ->where(MembershipPlanTable::SLUG, $plan[MembershipPlanTable::SLUG])
                ...
                DB::table(MembershipPlanTable::TABLE_NAME)->insert(array_merge($plan, [
